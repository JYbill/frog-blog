...
Now we have two options, is to dump the `database` manually or use some automation tool like `sqlmap` I am doing it with
`second` option so that `write up` will be short
